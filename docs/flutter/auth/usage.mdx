...
incognito mode on Google Chrome). Performing any of these operations means that the user won't be able to access the same account anymore.
...
* `url`: The deep link to embed and any additional state to be passed along. The link's domain has to be allow-listed in the Firebase Console list of authorized domains, which can be found by going to the Sign-in method tab (Authentication -> Sign-in method). The link will redirect the user to this URL if the app is not installed on their device and the app was not able to be installed.
...
Follow the setup process for Firebase Dynamic Links on Flutter in this [guide](https://firebase.flutter.dev/docs/dynamic-links/overview) and ensure you have
correctly integrated Dynamic Links for [Android](https://firebase.flutter.dev/docs/dynamic-links/android-integration) and [Apple](https://firebase.flutter.dev/docs/dynamic-links/apple-integration) devices.
